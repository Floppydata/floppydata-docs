...
These instructions – called **Proxy Generator** – define:
...
```mermaid
flowchart TD
    A[Your app] --> B[Request + Proxy Generator]
...
When you send requests through Floppydata, the target sees the **proxy exit IP** instead. Your real IP stays hidden behind the proxy layer.

This is what makes geo-targeting, rotation, and session control possible. Instead of exposing your own network identity, you control how requests appear from the outside.
...
Here is the same flow visually:
...
<Note>
  **When using Floppydata:**
  ...
  - **Proxy Generator** define how requests are routed
